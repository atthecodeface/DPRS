...
export class SimulationControls {
  ...
  populate_webpage_entries() {
    ...
    this.set_webpage_entry(
      "p_initial",
      this.parameters.probabilities.p_initial,
    );
    this.set_webpage_entry(
      "n_iterations",
  ...
  set_parameters_from_webpage_entries() {
    const growth_model = html.get_input_radio_checked(
      this.ele_id + "growth_model",
    );
    ...
    this.parameters.probabilities.p_conj = this.get_webpage_float(
      "p_conj",
      0,
      1,
    );
    this.parameters.probabilities.p_nbr = this.get_webpage_float("p_nbr", 0, 1);
    this.parameters.probabilities.p_diag = this.get_webpage_float(
      "p_diag",
      0,
      1,
    );
    this.parameters.probabilities.u_x = this.get_webpage_float(
      "u_x",
      -1e9,
      +1e9,
    );
    this.parameters.probabilities.p_initial = this.get_webpage_float(
      "p_initial",
      0,
      1,
    );
    this.parameters.settings.n_iterations = this.get_webpage_int(
      "n_iterations",
      0,
      1000000,
    );
    this.parameters.settings.sample_period = this.get_webpage_int(
      "sample_period",
      1,
      100000,
    );
    this.parameters.settings.random_seed = this.get_webpage_int(
      "random_seed",
      1,
      100000,
    );
  ...
  build_html() {
    ...
    // Dims
    {
      ...
      td.add_label("n_x", {
        classes: "sim_controls_labels dims_labels dims_label_nx",
      }).set_content("x:");
      td.add_input_text("n_x", "20", {
        id: this.ele_id + "n_x",
        classes: "sim_controls_inputs dims_inputs dims_input_nx",
      });
      if (dims >= 2) {
        const td = tr.add_ele("td");
        td.add_label("n_y", {
          classes: "sim_controls_labels dims_labels dims_label_ny",
        }).set_content("y:");
        td.add_input_text("n_y", "20", {
          id: this.ele_id + "n_y",
          classes: "sim_controls_inputs dims_inputs dims_input_ny",
        });
      }
      if (dims >= 3) {
        const td = tr.add_ele("td");
        td.add_label("n_z", {
          classes: "sim_controls_labels dims_labels dims_label_nz",
        }).set_content("z:");
        const x = td.add_input_text("n_z", "20", {
          id: this.ele_id + "n_z",
          classes: "sim_controls_inputs dims_inputs dims_input_nz",
        });
      }
    }
    ...
    // Probabilities
    {
      ...
      for (const [label, thing] of [
        ...
        const td = tr.add_ele("td");
        td.add_label(thing!, {
          classes: "sim_controls_labels probabilities_labels",
        }).set_content(label + ":");
        td.add_input_text(thing!, "0.5", {
          id: this.ele_id + thing,
          classes: "sim_controls_inputs probabilities_inputs",
        });
      }
    }
    {
    ...
    // Steps / slicing / seed
    {
      ...
      for (const [name, label, value] of [
        ...
        const td = tr.add_ele("td");
        td.add_label(name!, {
          classes: "sim_controls_labels stepsetc_labels",
        }).set_content(label + ":");
        td.add_input_text(name!, value!, {
          id: this.ele_id + name,
          classes: "sim_controls_inputs stepsetc_inputs",
        });
      }
    }
  ...
  get_webpage_float(id: string, min: number, max: number): number {
    return html.get_input_float(this.ele_id + id, min, max);
  }

  get_webpage_int(id: string, min: number, max: number): number {
