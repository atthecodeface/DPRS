# Contact Process

### CP in $d$ dimensions, according to Henkel et al (2008), is:

![alt text](Snippet_Henkel2008_CP1.png)
<!-- ![alt text](image-1.png)
![alt text](image-2.png) -->

Note that a site neighborhood here is strictly limited to the _nearest_ neighbors, so there are 2 in 1d, 4 in 2d, 6 in 3d, etc: hence the $2d$ factor.


### The algorithm is roughly:
 - Zero the iteration counter: $\, k=0$
 - Construct a $d$-dimensional lattice $\mathcal{L}$ of size $L$ along each axis 
 - Compute the total number of sites: $\, N = L^d$
 - Choose an initial site occupancy probability: $\, p_{0}$
 - Randomize the occupancy of all lattice sites $\{s_i\} \in  \mathcal{L}$ such that, for each $i$,  $s_i = \mathsf{Bern}(p_0)$
 - Make a list of *all* the lattice sites, not just the occupied ones: $\, \mathcal{S}_\mathsf{lattice}$
 - Randomize the order of this list: $\, \mathcal{S}_\textsf{lattice} \mapsto \mathcal{S}_\mathsf{randomized}$
 <!-- - Create an empty list $\mathcal{C}$ to record changes to site occupancies -->
 - Set a reporting interval: $\, \Delta{t}_\mathsf{report}$
 - Create an empty list $\mathcal{L}_\mathsf{report}$ to record the lattice after a sequence of changes over this interval
 - Create an empty list $\mathcal{T}_\mathsf{report}$ to record the report times 
 - Zero the timer:  $\, t = 0$
 - Zero the report interval timer:  $\, t_\mathsf{report} = 0$
 - Loop over $K$ iterations $k \in \{1, \dots, K\}$:
   - Consider the first site in the randomized list $\mathcal{S}_\mathsf{randomized}$ and label it $i$ with occupancy $s_i$
    - Remove this site $i$ from the list $\mathcal{S}_\mathsf{randomized}$
   - Find its neighborhood and label the sites $j \in \langle i \rangle$ with occupancy $s_j$
   - Count the number of occupied sites in the neighborhood: $\, n_i = \sum_{j\in \langle i \rangle} s_j$
   - Compute the _propagation_ rate: $\, w_i[0 \mapsto 1, n_i] = \lambda n_i/2 d$
   - Note the _annihilation_ rate: $\, w_i[1 \mapsto 0, n_i] =1$
   - Compute the _propagation probability_: $\, p_\mathsf{p} = \dfrac{w_i[0 \mapsto 1, n_i]}{1+\lambda} = \dfrac{\lambda n_i}{2d(1+\lambda)}$
   - Compute the _annihilation probability_: $\, p_\mathsf{a} = \dfrac{w_i[1 \mapsto 0, n_i]}{1+\lambda} = \dfrac{1}{(1+\lambda)}$
   <!-- - Note their sum is the chance of _change_ at this site $i$:  $\,\,$ $p_\mathsf{p}+p_\mathsf{a} = \dfrac{1+\lambda n_i/2d}{1+\lambda} \neq 1$ -->
    - Compute the time interval: $\, \Delta{t} = \dfrac{1}{(1+\lambda)N}$
    - Update the timer: $\, t \mapsto t + \Delta{t}$
        - note: the timer is incremented even if nothing happens
   - Check $s_i$
     - for an empty site $s_i$==$0$:
        -  compute a Bernoulli sample $b_\mathsf{p} = \mathsf{Bern}(p_\mathsf{p})$
        - if $b_\mathsf{p}$==$\mathsf{true}$:
           - designate the site as now occupied $s_i \mapsto 1$
           - modify the lattice accordingly $\mathcal{L} \mapsto \mathcal{L}(t)$
           <!-- - append the tuple $\{\textsf{propagation}, i, t\}$ to $\mathcal{C}$ -->
     - for an occupied site $s_i$==$1$:
        - compute a Bernoulli sample $b_\mathsf{a} = \mathsf{Bern}(p_\mathsf{a})$
        - if $b_\mathsf{a}$==$\mathsf{true}$:
           - designate the site as now empty $s_i \mapsto 0$
           - modify the lattice accordingly $\mathcal{L} \mapsto \mathcal{L}(t)$
           <!-- - append the tuple $\{\textsf{annihilation}, i, t\}$ to $\mathcal{C}$ -->
    - Reinsert the site $i$ at a _random_ location in the list $\mathcal{S}_\mathsf{randomized}$
    - Update the report timer: $\, t_\mathsf{report} \mapsto t_\mathsf{report} + \Delta{t}$
    - Check it's time to report, $t_\mathsf{report} \geq \Delta{}t_\mathsf{report}$; if so:
        - append the updated (or not) lattice $\mathcal{L}$ to the report list: $\, \mathcal{L}_\mathsf{report} \mapsto \mathcal{L}_\mathsf{report} + \mathcal{L}$
        - append the current time $t$ to the report time list $\mathcal{T}_\mathsf{report}$
        - reset the report interval timer $t_\mathsf{report} \mapsto 0$
- Return the report list of lattices $\mathcal{L}_\mathsf{report}$ together with the report time list $\mathcal{T}_\mathsf{report}$