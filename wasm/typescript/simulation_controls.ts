...
export class SimulationControls {
  ...
  populate_parameters() {
    ...
    const initial_center = html.get_input_checked(
      this.ele_id + "initial_center",
    );
  ...
  build_html() {
...

    {
      ...
      const td = tr.add_ele("td");
      td.add_input_checkbox(
        this.ele_id + "initial_center",
        this.ele_id + "initial_center",
        "sim_controls ",
      );
      td.add_label(this.ele_id + "initial_center").set_content(
        "Central cell (or: randomized)",
      );
    }
